...
const TokenRow: React.FC<{ token: Tokens[number] }> = ({ token }) => {
  ...
  const isChecked =
    checkedRecords[contract_address as `0x${string}`]?.isChecked ?? false;
  ...
  return (
    ...
    >
      ...
      <span style={{ marginLeft: '5px' }}>
        (worth {usdFormatter.format(quote)})
      </span>
    </div>
  );
};
...
export const GetTokens = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await httpFetchTokens(
        chain?.id as number,
        address as string,
      );
      if (response.data) {
        ...
        setCheckedRecords(
          Object.fromEntries(
            tokens.map((token: Tokens[number]) => [
      ...
      setError(
        `Unable to fetch tokens on ${chain?.name}. Please try again or switch networks.`,
      );
    }
    setLoading(false);
  }, [address, chain?.id]);
